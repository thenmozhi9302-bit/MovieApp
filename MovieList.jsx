...
const MovieList = ({type, title, emoji}) => {
  ...
  const fetchMovies = async () => {
    const response = await fetch (`https://api.themoviedb.org/3/movie/${type}?api_key=22dd492c77feba18a42f2d79485121bc`);
        ...
        <section className='movie_list' id={type}>
            <header className='movie_list_header'>
                ...
                <div className='movie_list_fs'>
                    <FilterGroup minRating={minRating} onRatingClick=
                    {handleFilter}
                    ratings={[8,7,6]}
                    />
